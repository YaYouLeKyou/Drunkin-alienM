...
if (isMobile) {
  ...
  function generateItemParticles(item) {
    ...
    for (let i = 0; i < numParticles; i++) {
      ...
      function render() {
        ...
        if (gamePlaying) {
          ...
          for (let i = items.length - 1; i >= 0; i--) {
            ...
            let collisionY = oscillatingY;
            let collisionHeight = scaledHeight;
            let collisionWidth = scaledWidth;

            if (item.type === 'beer') {
              collisionY = item.initialY;
              collisionHeight = item.height;
              collisionWidth = item.width;
            }

            if (
              cTenth < item.x + collisionWidth &&
              cTenth + size[0] > item.x &&
              flyHeight < collisionY + collisionHeight &&
              flyHeight + size[1] > collisionY
            ) {
      ...
      // Pause button functionality
      // (Removed duplicate declaration if already declared above)
